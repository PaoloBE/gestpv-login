...
public interface DireccionRepository extends JpaRepository<Direccion, Long> {
    ...
    @Query(value = "SELECT * FROM Admin.direccion WHERE persona_cliente_id_persona_cliente = :id", nativeQuery = true)
    Direccion findByPersonaId(@Param("id") Integer id);

    @Query(value = "UPDATE Admin.direccion SET ubigeo_id_ubigeo = :ubigeo WHERE persona_cliente_id_persona_cliente = :id AND direccion_estado = '1'", nativeQuery = true)
    @Modifying
    void updatePersonaDireccionUbigeo(@Param("id") Integer id, @Param("ubigeo") Integer ubigeo);

    @org.springframework.transaction.annotation.Transactional
    @Modifying
    @Query(value = "INSERT INTO Admin.direccion\n" +
            ...
            "VALUES\n" +
            "(:idPer,:desc,1,\n" +
            " NOW(),(SELECT id_ubigeo from Admin.ubigeo WHERE ubigeo_departamento= :dep AND ubigeo_provincia= :prov AND ubigeo_distrito= :dist),'','',\n" +
